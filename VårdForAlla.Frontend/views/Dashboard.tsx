...
const Dashboard: React.FC = () => {
  ...
  // FIXAT: Korrekt användning av useRoutines
  ...
  const quickLinks = [
    ...
    { label: 'Skapa Ny Rutin', path: '/manage', color: 'bg-green-600', icon: 'M12 4v16m8-8H4', adminOnly: false },
  ...
  // FIXAT: Filtrerar bort admin-länkar för vanliga användare
    ...
    <div className="space-y-12">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {visibleLinks.map((link) => (
          <Link 
            key={link.label} 
            to={link.path}
            className="group bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-blue-200 transition-all duration-300"
          >
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
             [...Array(3)].map(i => <div key={i} className="h-48 bg-white border border-slate-100 rounded-3xl animate-pulse"></div>)
            ...
            routines.slice(0, 3).map(routine => <RoutineCard key={routine.id} routine={routine} />)
          )}
        </div>
      </section>
    </div>
